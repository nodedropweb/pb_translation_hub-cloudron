...
- **Importing a real `mariadb-dump`-produced recovery file failed with `Variable 'autocommit' can't be set to the value of NULL`.** `mariadb-dump` wraps each table it dumps in session housekeeping — `SET @OLD_AUTOCOMMIT=@@AUTOCOMMIT, @@AUTOCOMMIT=0;` paired with a later `SET AUTOCOMMIT=@OLD_AUTOCOMMIT;`, plus `LOCK`/`UNLOCK TABLES`, charset/collation save-restore, etc. Confirmed live: `importSqlDump()`'s batching (0.4.15) split a large table's capture and restore lines across separate round trips, and MySQL 8 rejected the restore. Rather than enumerate every boilerplate variant a dump tool might emit — fragile, easy to miss one — the import now whitelists the one statement shape it actually needs (`INSERT INTO ...`) and skips everything else, instead of only skipping blank lines and `--` comments as before.

## [0.4.15] — 2026-08-23

### Fixed

- **The import of a real dataset (41k+ projects) timed out even after every earlier fix.** Confirmed live against a correct, full-size recovery dump from the actual production database: `504`, nginx log `upstream timed out (110: Connection timed out) while reading response header from upstream`. `importSqlDump()` executed one statement per round trip — tens of thousands of them for the real dataset — comfortably exceeding nginx's default 60s `proxy_read_timeout`, which was never explicitly set for `/api/`. Rewrote it to batch statements (300 per round trip) through a dedicated `multipleStatements` connection instead of the shared pool (kept isolated from every other query in the app — safe here specifically because the statements are our own trusted export format, never user input), cutting round-trip count by ~300x. Also explicitly raised `proxy_read_timeout`/`proxy_send_timeout` to 600s on `/api/` as a second line of defense.

## [0.4.14] — 2026-08-23

### Fixed

- **A real content backup rejected outright with a bare `413 Request Entity Too Large`.** `nginx/app.conf`'s `client_max_body_size` was `50M`, below multer's own `100M` `fileSize` limit in `server/index.js` — nginx sits in front of multer, so it rejected a real ~60 MB export (DB dump + category translations, from a live dataset of 41k+ projects) before multer's own, more specific error handling ever got a chance to run. Raised to `100M` to match.
